        ...
        {rows.map((row) => (
          <Card key={row.key} className="border-line bg-white">
            <CardContent className="space-y-2 p-4">
              <p className="text-xs uppercase tracking-[0.12em] text-foreground/70">{row.label}</p>
              <p className="font-display text-3xl uppercase leading-none text-secondary">
                {formatCurrency(row.ratePerVideo)}
              </p>
              <p className="text-xs text-foreground/60">par video validee</p>
              {row.isPlaceholder ? (
                <p className="text-[11px] text-amber-700">Tarif en cours d&apos;ajustement</p>
              ) : null}
            </CardContent>
          </Card>
        ))}
