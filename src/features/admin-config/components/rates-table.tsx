...
export function RatesTable({ rates }: RatesTableProps) {
  ...
  async function handleEnable(rate: VideoRate) {
    ...
    const success = await updateMutation.mutate({ videoType: rate.videoType, ratePerVideo: rate.ratePerVideo });
  ...
  return (
    <CardSection className="space-y-3">
      ...
      {updateMutation.lastSuccess && <p className="text-sm text-mint">Tarif mis a jour.</p>}
      {deleteMutation.lastSuccess && <p className="text-sm text-mint">Produit desactive.</p>}

      <div className="space-y-1.5">
        <div className="hidden grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-3 px-3 pb-1 text-[11px] uppercase tracking-[0.11em] text-foreground/45 sm:grid">
          <span>Produit</span>
          ...
          return (
            ...
            >
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-[11px] uppercase tracking-[0.08em] text-foreground/45">{rate.videoType}</p>
              </div>
                  ...
                  <div className="flex items-center justify-end gap-1.5">
                    ...
                    {rate.isPlaceholder ? (
                      ...
                      <Button
                        ...
                        title={disableBlocked ? "Au moins un produit doit rester actif." : undefined}
                        className="h-8 rounded-md px-2.5 text-[10px] tracking-[0.06em]"
                      >
                        Off
                      </Button>
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Ce type ne sera plus disponible pour les nouveaux uploads createur.
            </DialogDescription>
          </DialogHeader>
          ...
          {disableTarget ? (
            <div className="space-y-4 px-6 pb-6">
              <div className="rounded-xl border border-line bg-frost/60 px-4 py-3">
                ...
                <p className="mt-0.5 text-[11px] uppercase tracking-[0.08em] text-foreground/45">
              ...
              <div className="flex items-center justify-end gap-2">
                ...
                <Button
                  ...
                  disabled={deleteMutation.isPending}
                >
                  {deleteMutation.isPending ? "..." : "Desactiver"}
                </Button>
              </div>
            </div>
          ) : null}
