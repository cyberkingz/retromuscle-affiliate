    ...
    <html lang="fr">
      <body className={`${fontDisplay.variable} ${fontBody.variable}`}>
        ...
        {FB_PIXEL_ID ? (
          <>
            {/*
              Use a plain <script> with dangerouslySetInnerHTML instead of
              next/script afterInteractive — the latter serialises inline
              content as a JS string literal, which breaks on the newlines
              inside the pixel IIFE (SyntaxError: invalid token on appendChild).
