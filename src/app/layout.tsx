    ...
    <html lang="fr">
      <body className={`${fontDisplay.variable} ${fontBody.variable}`}>
        ...
        {FB_PIXEL_ID ? (
          <>
            {/*
              Load the pixel from a static public file (/fpixel.js) instead of
              inlining via dangerouslySetInnerHTML. Inline scripts in Server
              Components are embedded in the RSC payload (self.__next_f.push)
              where single-quotes inside the code cause a SyntaxError at parse
              time. An external src avoids RSC serialisation entirely.
              The nonce attribute satisfies our strict-dynamic CSP policy.
            */}
            <script
              id="fb-pixel"
              src="/fpixel.js"
